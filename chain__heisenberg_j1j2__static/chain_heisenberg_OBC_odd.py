...
###### setting up hamiltonian ######
    Jzzs = \
        [[J1,i,(i+1)%N] for i in range(N-1)] \
      + [[J2,i,(i+2)%N] for i in range(N-2)]
    Jpms = \
        [[0.5*J1,i,(i+1)%N] for i in range(N-1)] \
      + [[0.5*J2,i,(i+2)%N] for i in range(N-2)]
    Jmps = \
        [[0.5*J1,i,(i+1)%N] for i in range(N-1)] \
      + [[0.5*J2,i,(i+2)%N] for i in range(N-2)]
    static = [\
...
#        ene,vec = H.eigsh(time=0.0,which="SA",k=2)
        ene = H.eigsh(which="SA",k=100,return_eigenvectors=False); ene = np.sort(ene)
...
#        print(N,ene[0],ene-ene[0])
        print(N,ene[0],' '.join(map(str,ene[0:100]-ene[0])))
