...
## https://doi.org/10.1103/PhysRevB.99.054307
...
hopping1=[[-J1,j,(j+1)%N] for j in range(N-1)]
...
hopping2=[[-J2,j,(j+1)%N] for j in range(N-1)]
interaction2=[[0.5*U2,j,j] for j in range(N)]
#interactionV2=[[V2,j,(j+1)%N] for j in range(N)]
interactionV2=[[V2,j,(j+1)%N] for j in range(N-1)]
...
hopping=[[-J,j,(j+1)%N] for j in range(N-1)]
