...
#print(H2)
#
Ji=0.0 # hopping matrix element
Ui=1.0 # onsite interaction
hoppingi=[[-Ji,j,(j+1)%N] for j in range(N)]
interactioni=[[0.5*Ui,j,j] for j in range(N)]
potentiali=[[-0.5*Ui,j] for j in range(N)]
statici=[["+-",hoppingi],["-+",hoppingi],["nn",interactioni],["n",potentiali]]
dynamici=[]
no_checks=dict(check_symm=False, check_pcon=False, check_herm=False)
Hi=hamiltonian(statici,dynamici,static_fmt="csr",basis=basis_1d,dtype=np.float64,**no_checks).tocsr(time=0)
#Hi=hamiltonian(statici,dynamici,static_fmt="csr",basis=basis_1d,dtype=np.float64).tocsr(time=0)
#print(Hi)
#
z=2
Jf=0.1/z # hopping matrix element
Uf=1.0 # onsite interaction
hoppingf=[[-Jf,j,(j+1)%N] for j in range(N)]
interactionf=[[0.5*Uf,j,j] for j in range(N)]
potentialf=[[-0.5*Uf,j] for j in range(N)]
staticf=[["+-",hoppingf],["-+",hoppingf],["nn",interactionf],["n",potentialf]]
dynamicf=[]
no_checks=dict(check_symm=False, check_pcon=False, check_herm=False)
Hf=hamiltonian(staticf,dynamicf,static_fmt="csr",basis=basis_1d,dtype=np.float64,**no_checks).tocsr(time=0)
#Hf=hamiltonian(staticf,dynamicf,static_fmt="csr",basis=basis_1d,dtype=np.float64).tocsr(time=0)
#print(Hf)
#
# diagonalise Hi
ene0,vec0 = scipy.sparse.linalg.eigsh(Hi,which='SA',k=2)
print("J U ene/N",J0,U0,ene0[0]/N)
#print(vec0[:,0])
...
#enef = (np.conjugate(ret).dot(Hf.dot(ret)) / norm2).real / N
...
plt.plot(list_time,(-0.5)*np.array(list_ene1))
#plt.xscale("log")
plt.xlabel("$t$")
fig60.savefig("fig_N"+str(N)+"_adaga.png")
