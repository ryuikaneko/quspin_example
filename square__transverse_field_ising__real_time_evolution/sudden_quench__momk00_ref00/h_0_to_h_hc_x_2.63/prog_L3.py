...
g=-3.04438*2.63 # magnetic field strength
J0=0.0 # spin=spin interaction
g0=-1.0 # magnetic field strength (x)
J1=0.0 # spin=spin interaction
g1=-1.0 # magnetic field strength (z)
...
#
###### setting up bases ######
basis_2d = spin_basis_general(N_2d,kxblock=(T_x,0),kyblock=(T_y,0),pxblock=(P_x,0),pyblock=(P_y,0))
...
H1=hamiltonian(static1,[],basis=basis_2d,dtype=np.float64)
...
#print(ene0)
#print(vec0)
ene1,vec1 = scipy.sparse.linalg.eigsh(H1_csr,which='SA',k=2)
print(ene1)
print(vec1)
...
ret = scipy.sparse.linalg.expm_multiply((-1j)*H_csr,vec1[:,0],start=timei,stop=timef,num=Nsteps,endpoint=True)
print(ret)
...
print(mx)
mz = [(np.dot(np.conjugate(ret[i]),H1_csr.dot(ret[i])) / norm[i]).real / (-N_2d) for i in range(Nsteps)]
print(mz)
...
#plt.plot(time_steps_pi,mz)
plt.xlabel("$t$")
#plt.xlabel("$t/(\pi/4)$")
fig20.savefig("fig_mz.png")
#plt.show()
